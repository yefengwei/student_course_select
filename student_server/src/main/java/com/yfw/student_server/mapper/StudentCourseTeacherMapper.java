package com.yfw.student_server.mapper;

import com.yfw.student_server.entity.CourseTeacherInfo;
import com.yfw.student_server.entity.SCTInfo;
import com.yfw.student_server.entity.StudentCourseTeacher;
...
public interface StudentCourseTeacherMapper {
    ...
    @Select("SELECT DISTINCT sct.term FROM studentms.student_course_teacher sct")
    public List<String> findAllTerm();

    @Select("SELECT * FROM studentms.student_course_teacher WHERE sid = #{sct.sid} AND cid = #{sct.cid} AND tid = #{sct.tid} AND term = #{sct.term}")
    public List<StudentCourseTeacher> findBySCT(@Param("sct") StudentCourseTeacher studentCourseTeacher);

    @Insert("INSERT INTO studentms.student_course_teacher (sid, cid, tid, term) VALUES (#{s.sid}, #{s.cid}, #{s.tid}, #{s.term})")
    public boolean insert(@Param("s")StudentCourseTeacher studentCourseTeacher);

    @Update("UPDATE studentms.student_course_teacher as sct SET sct.grade = #{grade} WHERE sct.sid = #{sid} AND sct.tid = #{tid} AND sct.cid = #{cid} AND sct.term = #{term}")
    ...
    @Delete("DELETE FROM studentms.student_course_teacher as sct WHERE sid = #{sct.sid} AND tid = #{sct.tid} AND cid = #{sct.cid}")
